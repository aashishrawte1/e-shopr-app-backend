...
export class KKEventSheetSync {
	async sync() {
		console.log('STARTING SYNC OF KK SHEET');
    ...
    `;
		...
		let { rewardType = 'type1', actualNumberOfCoinsThatCanBeAwarded = 10000 } =
			await getNumberOfCoinsThatCanBeRewarded({ actionType: 'kk_season_2021', country });
		for (const [key, value] of Object.entries(usersToReward)) {
			if (key in dbUserRecordMap) {
				...
				if (shouldUpdate) {
					KKEventTableUpdateSQL += this.getSQL({
						mode: 'update',
						data: value,
		...
		if (Object.values(errors).length > 0) {
			sendEmail({
				...
				message: Object.values(errors)
					...
					.join('</br>'),
				subject: 'error - the_excel sheet has some issue',
			});
		}
		// If has users
      ...
      `;
		...
		for (const u of userDetails) {
			await Database.query.task(async (tx) => {
				...
				let activity: UserCoinActivity = {
					...
					actionType: 'kk_season_2021',
					data: {
						email: u.email,
					},
					coinsRewarded: Math.round(coinsToAdd),
					deviceInfo: null,
					sessionId: 'kk_season_2021',
				};
		...
		await Database.query.none(KKEventTableUpdateSQL);
	}
...
export const getUserStatForKKEvent2021_API = async (request: Request, response: Response) => {
	let hasError = false;
